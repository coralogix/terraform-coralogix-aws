...
| <a name="input_iam_path"></a> [iam\_path](#input\_iam\_path) | Path under which to create the IAM policy. When `null` (default) the policy keeps its existing `/coralogix/` path; set a value to override it. | `string` | `null` | no |
