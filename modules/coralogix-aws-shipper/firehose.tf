...
resource "aws_iam_role_policy" "cloudwatch_metrics_policy" {
  count = var.telemetry_mode == "metrics" ? 1 : 0
  name  = "firehose_access_policy_${random_string.lambda_role.result}"
...
resource "aws_cloudwatch_metric_stream" "cloudWatch_metric_stream" {
  count         = var.telemetry_mode == "metrics" ? 1 : 0
  name          = "metrics-firehose-shipper-test-coralogix-metric-stream-${random_string.lambda_role.result}"
...
resource "aws_iam_role" "cloudwatch_metrics_role" {
  ...
  name = "metrics-firehose-shipper-test-FirehoseAccessRole-${random_string.lambda_role.result}"
