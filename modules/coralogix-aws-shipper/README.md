...
| <a name="create_execution_role"></a> [create_execution_role](#input\_create_execution_role) | Explicitly control whether the module creates its own IAM execution role. When null (default), auto-detects based on execution_role_arn and execution_role_name. Set to false when providing a computed execution_role_arn to avoid plan-time errors. | `bool` | null | no |
| <a name="execution_role_arn"></a> [execution_role_arn](#input\_execution_role_arn) | The ARN of an existing IAM role to use as the Lambda execution role. When provided, the module will not create its own role. When providing a computed ARN (e.g. from a resource in the same configuration), also set create_execution_role = false to avoid plan-time errors. This variable and execution_role_name are mutually exclusive (use one OR the other, not both). If both are provided, execution_role_arn takes priority. | `string` | n/a | no |
| <a name="execution_role_name"></a> [execution_role_name](#input\_execution_role_name) | The name of an existing IAM role to use as the Lambda execution role (looked up via data source). When provided, the module will not create its own role. This is the recommended approach when referencing a role created in the same Terraform configuration, as the name is typically known at plan time even when the ARN is computed. This variable and execution_role_arn are mutually exclusive (use one OR the other, not both). If both are provided, execution_role_arn takes priority. | `string` | n/a | no |
