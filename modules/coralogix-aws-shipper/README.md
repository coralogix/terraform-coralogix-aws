...
| <a name="subsystem_name"></a> [subsystem\_name](#input\_subsysten_\_name) | The [name](https://coralogix.com/docs/application-and-subsystem-names/) of your subsystem. for dynamic value from the log you should use `$.my_log.field` for CloudWatch log group leave empty. This option is not supported since version `1.1.0` for the [source code](https://github.com/coralogix/coralogix-aws-shipper/blob/master/CHANGELOG.md) | string | n\a | yes |
...
Note that metadata is not added by default. You must specify the metadata keys you want in the `add_metadata` parameter.
For example, if you want to add the bucket name and key name to the log message, you would set the `add_metadata` parameter to `s3.object.key,s3.bucket`.
...
```
For example, if you want to use the bucket name as the subsystem name, you would set the `subsystem_name` parameter to:
```
{{ s3.bucket }}
```
If you want to use the log group name as the application name, you would set the `application_name` parameter to:
```
{{ cw.log.group }}
```
If you only want to use part of the metadata value, you can use a regular expression to extract the desired part. For example, If we have an `s3.object.key` value of `AWSLogs/112322232/ELB1/elb.log` and we want to extract the last part of the key as the Subsystem name, we would set the `subsystem_name` parameter to:
```
