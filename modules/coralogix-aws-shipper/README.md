...
| <a name="create_execution_role"></a> [create_execution_role](#input\_create_execution_role) | Whether the module should create its own IAM role for the Lambda function. Automatically set to false when `execution_role_arn` or `execution_role_name` is provided. | `bool` | `true` | no |
