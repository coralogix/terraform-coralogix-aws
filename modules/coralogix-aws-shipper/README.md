...
| <a name="execution_role_arn"></a> [execution_role_arn](#input\_execution_role_arn) | The ARN of the IAM role to use as the Lambda execution role. Prefer this over execution_role_name so Terraform maintains the dependency graph (e.g. aws_iam_role.foo.arn). When the role is managed in the same Terraform configuration, this ensures proper dependency tracking without requiring manual depends_on. | `string` | n/a | no |
| <a name="execution_role_name"></a> [execution_role_name](#input\_execution_role_name) | The name of a user defined role to be used as the execution role for the Lambda function. This triggers a data lookup by name. Prefer execution_role_arn when the role is managed in the same Terraform configuration for better dependency tracking. | `string` | n/a | no |
