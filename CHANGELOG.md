...
- Traces mode fails at plan time on: a non-CloudWatch `integration_type`, `log_groups` other than `["aws/spans"]`, a Kinesis, Kafka or MSK trigger, `enable_dlq`, or `subnet_ids` without an `otlp_endpoint`.
