...
### 💡 Enhancements 💡
- Added `execution_role_arn` variable to allow providing a custom execution role by ARN. Supports computed ARNs when paired with `execution_role_name`.
- Added centralized execution role resolution logic in `local.tf` with `local.lambda_execution_role_arn` and `local.lambda_execution_role_name` locals.
